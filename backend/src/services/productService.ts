...
const initialCategories = [
  {
    name: 'Cà phê',
    code: MASTER_CODES.PRODUCT_CATEGORY.COFFEE,
    description: 'Các món cà phê pha phin, pha máy',
    sortOrder: 1,
  },
  {
    name: 'Trà',
    code: MASTER_CODES.PRODUCT_CATEGORY.TEA,
    description: 'Trà hoa quả, trà sữa, matcha',
    sortOrder: 2,
  },
  {
    name: 'Bánh ngọt',
    code: MASTER_CODES.PRODUCT_CATEGORY.CAKE,
...
const getProducts = async (queryParamsOrBody: any = {}) => {
  try {
    ...
    if (sortConditions && sortConditions.length > 0) {
      ...
      sortDir =
        (sortConditions[0].sortOrder || 'asc').toUpperCase() === 'DESC'
          ? 'DESC'
          : 'ASC'
    } else if (queryParamsOrBody.sortBy) {
      rawSortCol = queryParamsOrBody.sortBy
      sortDir =
        (queryParamsOrBody.sortOrder || 'asc').toUpperCase() === 'DESC'
          ? 'DESC'
          : 'ASC'
    }
    ...
    const categoryFilter =
      searchConditions.category || queryParamsOrBody.category
    if (
      categoryFilter &&
      categoryFilter !== MASTER_CODES.FILTER.ALL &&
      categoryFilter !== 'all'
    ) {
    ...
    const formattedProducts = rows.map((p: any) => {
      ...
      return {
        ...
        categoryCode:
          p.category?.name === 'Trà'
            ? MASTER_CODES.PRODUCT_CATEGORY.TEA
            : p.category?.name === 'Bánh ngọt'
              ? MASTER_CODES.PRODUCT_CATEGORY.CAKE
              : MASTER_CODES.PRODUCT_CATEGORY.COFFEE,
        ...
        statusCode:
          p.status === 'Tạm ngừng'
            ? MASTER_CODES.PRODUCT_STATUS.SUSPENDED
            : MASTER_CODES.PRODUCT_STATUS.IN_BUSINESS,
        status: p.status,
        img: p.imageUrl,
        unit: p.unit,
        recipeItems,
  ...
  try {
    if (db.sequelize.getDialect() === 'postgres') {
      try {
        ...
        await db.sequelize.query(
          `SELECT setval('product_recipes_id_seq', (SELECT COALESCE(MAX(id), 1) FROM product_recipes));`
        )
      } catch (seqErr) {
    ...
    if (
      !data.recipeItems ||
      !Array.isArray(data.recipeItems) ||
      data.recipeItems.length === 0
    ) {
      throw new Error(
    ...
    for (const r of data.recipeItems) {
      if (!r.amount || Number(r.amount) <= 0) {
        throw new Error(
          'Định lượng nguyên liệu trong công thức BOM phải lớn hơn 0'
        )
      }
    }
    ...
    if (!categoryId && data.category) {
      let categoryRecord = await db.Category.findOne({
    ...
    const newProduct = await db.Product.create({
      ...
      imageUrl: data.imageUrl || data.img || null,
      isActive: true,
      isDeleted: false,
    })
  ...
  try {
    ...
    if (!product) {
      product = await db.Product.findOne({
        where: { code: String(id), isDeleted: false },
      })
    }
    ...
    if (data.sellingPrice !== undefined || data.price !== undefined)
      updateFields.sellingPrice = Number(data.sellingPrice ?? data.price) || 0
    ...
    if (data.imageUrl !== undefined || data.img !== undefined)
      updateFields.imageUrl = data.imageUrl || data.img
