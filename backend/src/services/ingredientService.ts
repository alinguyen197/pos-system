...
const initialIngredients = [
  { code: 'NL-001', name: 'Cà phê Robusta Hạt', categoryCode: MASTER_CODES.INGREDIENT_CATEGORY.COFFEE_BEANS, categoryName: 'Cà phê hạt', quantity: 15.5, minQuantity: 5, unit: 'kg', costPerUnit: 180000 },
  { code: 'NL-002', name: 'Cà phê Arabica Hạt', categoryCode: MASTER_CODES.INGREDIENT_CATEGORY.COFFEE_BEANS, categoryName: 'Cà phê hạt', quantity: 1.5, minQuantity: 5, unit: 'kg', costPerUnit: 260000 },
  { code: 'NL-003', name: 'Sữa tươi thanh trùng 1L', categoryCode: MASTER_CODES.INGREDIENT_CATEGORY.MILK_CREAM, categoryName: 'Sữa & Kem', quantity: 2, minQuantity: 10, unit: 'lít', costPerUnit: 34000 },
  { code: 'NL-004', name: 'Sữa đặc Ngôi Sao', categoryCode: MASTER_CODES.INGREDIENT_CATEGORY.MILK_CREAM, categoryName: 'Sữa & Kem', quantity: 24, minQuantity: 12, unit: 'lon', costPerUnit: 22000 },
  { code: 'NL-005', name: 'Siro Đào Monin 700ml', categoryCode: MASTER_CODES.INGREDIENT_CATEGORY.SYRUP_SUGAR, categoryName: 'Siro & Đường', quantity: 0.4, minQuantity: 2, unit: 'lít', costPerUnit: 210000 },
]
...
const getIngredients = async (queryParamsOrBody: any = {}) => {
  try {
    ...
    const { queryOptions, page, pageSize } = buildListQuery(
      { sortConditions, pagination, sortBy: queryParamsOrBody.sortBy, sortOrder: queryParamsOrBody.sortOrder },
      'id',
      'ASC'
    )
    ...
    const rawKeyword = searchConditions.keyword || queryParamsOrBody.keyword || queryParamsOrBody.search || queryParamsOrBody.q || ''
    ...
    const formatted = rows.map((item: any) => {
      const statusCode = getStockStatusCode(item.quantity, item.minQuantity)
    ...
    const allStockItems = await db.StockItem.findAll({ where: { isDeleted: false } })
    ...
    const summary = {
      ...
      totalValue,
      safe: allStockItems.filter((i: any) => getStockStatusCode(i.quantity, i.minQuantity) === MASTER_CODES.STOCK_STATUS.SAFE).length,
      ...
      outOfStock: allStockItems.filter((i: any) => getStockStatusCode(i.quantity, i.minQuantity) === MASTER_CODES.STOCK_STATUS.OUT_OF_STOCK).length,
    }
    ...
    return {
      items,
      pagination: buildPaginationResponse(initialIngredients.length, page, pageSize),
    }
  }
}
...
const createIngredient = async (data: any) => {
  try {
    if (db.sequelize.getDialect() === 'postgres') {
      try {
        await db.sequelize.query(`SELECT setval('stock_items_id_seq', (SELECT COALESCE(MAX(id), 1) FROM stock_items));`)
      } catch (seqErr) {
    ...
    const quantity = data.quantity !== undefined ? Number(data.quantity) : (data.initialStock !== undefined ? Number(data.initialStock) : 0)
    const minQuantity = data.minQuantity !== undefined ? Number(data.minQuantity) : (data.minStock !== undefined ? Number(data.minStock) : 0)
    const costPerUnit = data.costPerUnit !== undefined ? Number(data.costPerUnit) : (data.costPrice !== undefined ? Number(data.costPrice) : 0)
...
const updateIngredient = async (id: string | number, data: any) => {
  try {
    let item = await db.StockItem.findOne({ where: { id: Number(id) || 0, isDeleted: false } })
    if (!item && typeof id === 'string') {
      item = await db.StockItem.findOne({ where: { code: id, isDeleted: false } })
    }
    ...
    if (data.category !== undefined) updateData.category = categoryMap[data.category] || data.category
    ...
    if (data.minQuantity !== undefined) updateData.minQuantity = Number(data.minQuantity)
    else if (data.minStock !== undefined) updateData.minQuantity = Number(data.minStock)
    if (data.costPerUnit !== undefined) updateData.costPerUnit = Number(data.costPerUnit)
    else if (data.costPrice !== undefined) updateData.costPerUnit = Number(data.costPrice)
